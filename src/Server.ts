...
export default class Server {
	...
	constructor(serverOptions: ServerOptions) {
		...
		this.lessonController = new LessonController(this.sql);

		this.userController.registerRoutes(this.router);
		this.authController.registerRoutes(this.router);
		this.lessonController.registerRoutes(this.router);
		...
		this.router.get("/error", (req: Request, res: Response) => {
			let searchParams: URLSearchParams = req.getSearchParams();
			let statusCode: number = parseInt(searchParams.get("code")!) || StatusCode.BadRequest;
	...
	handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
		...
		response.setCookie(request.session.cookie);
